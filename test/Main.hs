...
import Prettyprinter (Pretty (pretty), layoutPretty)
...
strToTemp :: String -> Ident 'Temporary
strToTemp = Ident @'Temporary . ShortText.fromString
...
goldenTests =
    testGroup
        ...
        , t "long_print_call" longPrintCall
        ]
  where
    t name value =
        goldenVsAction
            ...
            ("golden" </> name <.> "qbe")
            (pure $ pretty value)
            (renderStrict . layoutPretty layoutOptions)

valInt :: Int -> Val
valInt i
    | i >= 0 = ValConst $ CInt False $ fromIntegral i
...
longPrintCall =
    let
        createArg :: Int -> Arg
        createArg i = Arg (AbiBaseTy Long) $ ValTemporary (strToTemp $ "long_variable_name_" ++ show i)
     in
        Call Nothing (ValGlobal "printf") Nothing (map createArg [1 .. 10]) []
