...
module Language.QBE (
    ...
    -- * LayoutOptions, because page width must be Unbounded
    layoutOptions,
) where
...
instance Pretty Const where
    pretty (CInt negative int)
        | negative = pretty '-' <> pretty int
...
instance Pretty DataDef where
    pretty (DataDef linkage ident alignment fields) =
        vsep
            [ vsep $ pretty <$> linkage
            , hsep $
                ("data" <+> pretty ident <+> equals)
...
instance Pretty FuncDef where
    pretty (FuncDef linkage abiTy ident env params variadic blocks) =
        vsep
            [ vsep $ pretty <$> linkage
            , "function"
                <+> pretty abiTy
...
data Block
    ...
    | CommentedBlock String Block
    deriving (Show, Eq)

instance Pretty Block where
    pretty (Block ident phis insts jump) =
        hang 4 $
            vsep $
                concat
                    [ [pretty ident]
    ...
    pretty (CommentedBlock comment block) =
        vsep [pretty '#' <+> pretty comment, pretty block]
...
data Jump
    ...
    | -- | Halt
      Hlt
    | -- | Let backend insert the jump instruction to the next block
      Next
    deriving (Show, Eq)
...
instance Pretty Inst where
    ...
    pretty (Call assignment func env args variadics) =
        hsep $
...
instance Pretty Program where
    pretty (Program typeDefs dataDefs funcDefs) =
        vsep $
            concat
                [ pretty <$> typeDefs
                , pretty <$> dataDefs
